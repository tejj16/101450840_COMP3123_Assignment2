...
const App = () => {
  ...
  useEffect(() => {
    const userToken = localStorage.getItem('userToken'); // Example of checking login status
    if (userToken) {
      setIsLoggedIn(true);
    }
  }, []);
  ...
  return (
    <Router>
      <Routes>
        {/* Redirect logic */}
        <Route path="/" element={isLoggedIn ? <Navigate to="/employee" /> : <Navigate to="/login" />} />

        <Route
          path="/signup"
          element={isLoggedIn ? <Navigate to="/employee" /> : <Signup onSignup={() => setIsLoggedIn(true)} />}
        />
        <Route
          path="/login"
          element={isLoggedIn ? <Navigate to="/employee" /> : <Login onLogin={() => setIsLoggedIn(true)} />}
        />
        <Route
          path="/employee"
